...
<head>
  ...
  <title>
    <?php echo $settings_r['site_title'] ?> - BOOKINGS
  </title>
</head>
        ...
        if ($data['booking_status'] == 'booked') {
          ...
          if ($data['arrival'] == 1) {
            ...
            if ($data['rate_review'] == 0) {
              $btn .= "<button type='button' onclick='review_room($data[booking_id],$data[room_id])' data-bs-toggle='modal' data-bs-target='#reviewModal' class='btn btn-dark btn-sm shadow-none ms-2'>Review & Rating</button>";
          ...
          if ($data['refund'] == 0) {
            $btn = "<span class='badge bg-primary'>Refund is in process!</span>";
          } else {
            $btn = "<a href='generate_pdf.php?gen_pdf&id=$data[booking_id]' class='btn btn-dark btn-sm shadow-none'>Download PDF</a>";
          }
        }
            ...
            <div class='col-md-4 px-4 mb-4'>
              <div class='bg-white p-3 rounded shadow-sm'>
                 ...
                 <p>₹$data[price]per night</p>
                  ...
                  <p>
                    <b>Amount: </b> ₹$data[price]per night <br><br>
        ...
        <form id="review-form">
          <div class="modal-header">
            <h5 class="modal-title d-flex align-items-center">
              <i class="bi bi-chat-square-heart-fill fs-3 me-2"></i> Review & Rating
            </h5>
  ...
  <script>
    ...
    function cancel_booking(id) {
      if (confirm('Are you sure you want to cancel this booking?')) {
        ...
        xhr.open("POST", "ajax1/cancel_booking1.php", true);
    ...
    review_form.addEventListener('submit',function(e) {
      ...
      xhr.open("POST", "ajax1/review_room.php", true);

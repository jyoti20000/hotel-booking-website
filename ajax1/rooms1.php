   ...
   session_start();
    ...
    {    
        ...
        if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
        {
            ...
            {
              echo "<h3 class='text-center text-danger'>Invalid Dates! <br><br> You can not checkout on same days</h3>";
              exit;
            }
            else if($checkout_date < $checkin_date)
            {
                echo "<h3 class='text-center text-danger'>Invalid Dates! <br><br> Checkout date is earlier than checkin date!</h3>";
                exit;
            }
            else if($checkin_date < $today_date)
            {
              echo "<h3 class='text-center text-danger'>Invalid Dates! <br><br> Checkin date is earlier than todays date!</h3>";
              exit;
            }
        }
        ...
        // count the no. of rooms and output variable to store rooms cards
        ...
        while ($room_data = mysqli_fetch_assoc($room_res)) 
        {
            ...
            // get facility of room with filters

            $fac_count = 0;
  
            $fac_q = mysqli_query($conn, "SELECT f.name, f.id FROM `facilities` f 
            INNER JOIN `room_facilities` rfac 
            ON f.id = rfac.facilities_id

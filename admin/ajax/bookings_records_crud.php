...
if (isset($_POST['get_bookings'])) {
  ...
  $limit = 2;          // implementation of pagination
  $page = $frm_data['page'];
  $start = ($page-1)*$limit;   // it is a type to show the records in page..

   $query = "SELECT bo.*, bd.* FROM `booking_order` bo 
   INNER JOIN `booking_details1` bd ON bo.booking_id = bd.booking_id
   WHERE ((bo.booking_status='booked' AND bo.arrival=1) 
   OR (bo.booking_status='cancelled' AND bo.refund=1))
   AND (bo.user_id LIKE ? OR bd.phonenum LIKE ? OR bd.user_name LIKE ?)
   ORDER BY bo.booking_id DESC";

  $res = select($query,["%$frm_data[search]%","%$frm_data[search]%","%$frm_data[search]%"],'sss');
   
  // below two line is use to show the data means kha se kha tk data ko dikhana hai
  ...
  if($total_rows>$limit)
  {
     $total_pages = ceil($total_rows/$limit);    // ceil convert the flot no into integer and roundout its next higher values
